Feature: Buy Mobile Phone Feature

Scenario: A user should be able to buy a mobile phone successfully on amazon
Given user is on landing page of amazon
When he clicks on mobile tab
And he hovers the pointer over Mobile & Accessories tab
And he clicks on Apple in Sub-menu
And he clicks on first available phone
And he switches focus on new tab
And he clicks on Buy now button
Then he must able to purchase the mobile phone successfully